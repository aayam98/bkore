...
export const mapCart = (cart: Cart): SingleItemCartProps[] => {
  const res = cart.lines.edges.map((item) => ({
    ...
    // saved: +item.node.estimatedCost.subtotalAmount.amount,
    ...
    lineId: item.node.id,
    cartId: cart.id,
    tags:item.node.merchandise.product.tags,
    currentlyNotInStock: item.node.merchandise.currentlyNotInStock,
